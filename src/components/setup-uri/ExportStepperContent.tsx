...
import { ArrowRight, CheckCircle2, ChevronDown, RotateCcw, AlertTriangle } from "lucide-react";
...
export const ExportStepperContent: React.FC<ExportStepperContentProps> = ({ pluginVersion }) => {
  ...
  const isPasswordValid =
    password.length >= MIN_PASSPHRASE_LENGTH && password === confirmPassword;
      ...
      {currentStep === 0 && (
        <div className="tw-flex tw-flex-col tw-gap-5">
          ...
          <div className="tw-flex tw-flex-col tw-gap-1.5">
            ...
            />
            {errors.password && (
              <p className="tw-text-xs tw-text-error">{errors.password}</p>
            )}
            {!errors.password &&
          ...
          <div className="tw-flex tw-flex-col tw-gap-1.5">
            ...
            />
            {errors.confirm && (
              <p className="tw-text-xs tw-text-error">{errors.confirm}</p>
            )}
          </div>
      ...
      {currentStep === 1 && (
        <div className="tw-flex tw-flex-col tw-gap-5">
          ...
          <div className="tw-flex tw-items-start tw-gap-2 tw-rounded-md tw-bg-callout-warning/10 tw-p-3 tw-text-xs tw-text-callout-warning">
            <AlertTriangle className="tw-mt-0.5 tw-size-4 tw-shrink-0" />
            <p className="tw-leading-relaxed">
              This URI contains your encrypted API keys. It may be captured by clipboard managers, OS
              logs, or other apps. Share it securely and delete copies after use.
            </p>
          </div>
          ...
          <Collapsible>
            ...
            <CollapsibleContent className="tw-mt-3">
              <p className="tw-mb-2 tw-text-xs tw-text-muted">
