...
interface ImportStepperContentProps {
  ...
  onNotifyManualCopy: (folders: { customPromptsFolder?: string; userSystemPromptsFolder?: string; memoryFolderName?: string }) => void;
...
export const ImportStepperContent: React.FC<ImportStepperContentProps> = ({
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const importedSettings =
        cachedSettingsRef.current ?? (await applySetupUri(extractPayloadFromUri(setupURI), password)).settings;
  ...
  return (
    <div className="tw-flex tw-flex-col tw-gap-6">
      <p className="tw-text-sm tw-leading-relaxed tw-text-muted">
        ...
        <div className="tw-flex tw-flex-col tw-gap-5">
          <div className="tw-flex tw-items-start tw-gap-2 tw-rounded-md tw-bg-callout-warning/10 tw-p-3 tw-text-xs tw-text-callout-warning">
            ...
            <p className="tw-leading-relaxed">
              Only import Setup URIs from sources you trust. A malicious URI could overwrite your API
              keys and redirect requests to unauthorized servers.
            </p>
          </div>
      ...
      {currentStep === 1 && (
        <div className="tw-flex tw-flex-col tw-gap-5">
          ...
          {errorMessage && (
            <p className="tw-rounded tw-bg-error tw-p-2 tw-text-xs tw-text-error">{errorMessage}</p>
          )}
        ...
        <div className="tw-flex tw-flex-col tw-gap-5">
          ...
          {cachedMetaRef.current && (
            <div className="tw-flex tw-flex-col tw-gap-1 tw-rounded-md tw-bg-secondary tw-p-3 tw-text-xs tw-text-muted">
              <span>
                <span className="tw-font-medium tw-text-normal">Source:</span>{" "}
                Copilot v{cachedMetaRef.current.pluginVersion}
              </span>
              <span>
