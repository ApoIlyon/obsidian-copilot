...
import { persistSettings, loadSettingsWithKeychain, flushPersistence } from "@/services/settingsPersistence";
  ...
  async onload(): Promise<void> {
    ...
    this.settingsUnsubscriber = subscribeToSettingsChange(async (prev, next) => {
      try {
        ...
        new Notice(
          "Copilot failed to save settings. Check logs and try again."
        );
      }
