...
describe("crypto.ts", () => {
  ...
  describe("decryptWithPassphrase format validation", () => {
    it.each([
      ...
      "should throw reason='$reason' for $name",
      async ({ encoded, reason, msgSubstr }) => {
  ...
  describe("assertSetupUriPassphrase", () => {
    ...
    it("should not throw for passphrases at or above minimum length", () => {
      expect(() => assertSetupUriPassphrase("a".repeat(MIN_SETUP_URI_PASSPHRASE_LENGTH))).not.toThrow();
      expect(() => assertSetupUriPassphrase("a-very-long-passphrase-that-exceeds-minimum")).not.toThrow();
    });
  });
