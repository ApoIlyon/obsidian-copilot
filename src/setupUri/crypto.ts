...
/**
 * Decompress with a hard output size limit to prevent zip-bomb DoS.
 *
 * Uses pako's streaming Inflate API so we can abort as soon as decompressed
 * output exceeds the threshold, without materializing the entire result.
 *
...
function inflateWithLimit(compressed: Uint8Array, maxBytes: number): Uint8Array {
  ...
  // abort decompression early. Use a flag to skip chunk accumulation instead.
...
export async function decryptWithPassphrase(
  ...
  const iterations = readUint32LE(raw, 1) >>> 0; // Reason: unsigned shift ensures non-negative
