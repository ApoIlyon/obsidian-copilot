...
import { generateSetupUri, extractPayloadFromUri, applySetupUri, validateSetupUri } from "./setupUri";
...
describe("extractPayloadFromUri", () => {
  it("should extract payload from a full obsidian:// URI", () => {
    const result = extractPayloadFromUri(
      "obsidian://copilot-setup?payload=ABC123&v=1"
    );
...
describe("generateSetupUri", () => {
  const mockSettings = {
    ...
    activeModels: [
      { name: "gpt-4", apiKey: "enc_model-key", enabled: true },
    ],
    activeEmbeddingModels: [
      { name: "embed-v1", apiKey: "enc_embed-key" },
    ],
  };
  ...
  it("should throw when getDecryptedKeyOrThrow fails", async () => {
    ...
    await expect(generateSetupUri("password123", "3.2.3")).rejects.toThrow(
      /Failed to decrypt/
    );
  });
});
...
describe("applySetupUri", () => {
  ...
  it("should strip prototype-pollution keys at all nesting depths", async () => {
    ...
    (decryptWithPassphrase as jest.Mock).mockResolvedValue(
      JSON.stringify(maliciousEnvelope)
    );
